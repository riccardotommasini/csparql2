...
public class JenaContinuousQueryExecution extends Observable implements Observer, ContinuousQueryExecution<Graph, Graph, Binding> {
    ...
    public void update(Observable o, Object arg) {
        Long now = (Long) arg;
        sds.materialize(now);
        Collection<SolutionMapping<Binding>> r = r2r.eval(now);
        r.forEach(ib -> {
    ...
    private Table apply2(Binding eval, Long now) {
        ...
        Binding timestamp = BindingFactory.binding(Var.alloc("eventTime"), o);
        ...
        timestamp = BindingFactory.binding(Var.alloc("processingTime"), o);
