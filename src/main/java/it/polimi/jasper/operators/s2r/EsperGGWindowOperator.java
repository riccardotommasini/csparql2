...
public class EsperGGWindowOperator implements StreamToRelationOperator<Graph, Graph> {
    ...
    public TimeVarying<Graph> apply(WebDataStream<Graph> s) {
        EsperGGWindowAssigner windowAssigner = EPLFactory.getWindowAssigner(tick, maintenance, report, eventtime, s.getURI(), wo.getStep(), wo.getRange(), wo.getUnitStep(), wo.getUnitRange(), wo.getType(), time);
